import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';
import 'package:toyotamobile/Styles/color.dart';

// 40px line height / 25px font size = 1.6

class TextStyleList {
  //34
  static final TextStyle headingbar = GoogleFonts.kanit(
      fontSize: 34,
      fontWeight: FontWeight.w600,
      color: black13,
      height: 1.6,
      decoration: TextDecoration.none);
  static final TextStyle headingbar2 = GoogleFonts.kanit(
      fontSize: 34,
      fontWeight: FontWeight.w600,
      color: red7,
      height: 1.6,
      decoration: TextDecoration.none);
  //25
  static final TextStyle heading = GoogleFonts.kanit(
      fontSize: 25,
      fontWeight: FontWeight.bold,
      color: black2,
      height: 1.6,
      decoration: TextDecoration.none);

  //22
  static final TextStyle subheading = GoogleFonts.kanit(
      fontSize: 22,
      fontWeight: FontWeight.w500,
      color: black8,
      decoration: TextDecoration.none);
  //20

  static final TextStyle headtitle = GoogleFonts.kanit(
      fontSize: 20,
      fontWeight: FontWeight.w500,
      color: black8,
      decoration: TextDecoration.none);
  static final TextStyle headtitle2 = GoogleFonts.kanit(
      fontSize: 20,
      fontWeight: FontWeight.w400,
      color: black8,
      decoration: TextDecoration.none);

//18
  static final TextStyle title1 = GoogleFonts.kanit(
      fontSize: 18,
      fontWeight: FontWeight.w500,
      color: black,
      decoration: TextDecoration.none);
  static final TextStyle title2 = GoogleFonts.kanit(
      fontSize: 18,
      fontWeight: FontWeight.w500,
      color: black4,
      decoration: TextDecoration.none);
  static final TextStyle title3 = GoogleFonts.kanit(
      fontSize: 18,
      fontWeight: FontWeight.w400,
      color: black4,
      decoration: TextDecoration.none);
  static final TextStyle title4 = GoogleFonts.kanit(
      fontSize: 18,
      fontWeight: FontWeight.w600,
      color: black4,
      decoration: TextDecoration.none);
  static final TextStyle title5 = GoogleFonts.kanit(
      fontSize: 18,
      fontWeight: FontWeight.w600,
      color: green1,
      decoration: TextDecoration.none);

//16
  static final TextStyle subtitle1 = GoogleFonts.kanit(
      fontSize: 16,
      fontWeight: FontWeight.w500,
      color: black4,
      decoration: TextDecoration.none);
  static final TextStyle subtitle2 = GoogleFonts.kanit(
      fontSize: 16,
      fontWeight: FontWeight.w400,
      color: black7,
      decoration: TextDecoration.none);
  static final TextStyle subtitle3 = GoogleFonts.kanit(
      fontSize: 16,
      fontWeight: FontWeight.w500,
      color: black7,
      decoration: TextDecoration.none);
  static final TextStyle subtitle4 = GoogleFonts.kanit(
      fontSize: 16,
      fontWeight: FontWeight.w600,
      color: black7,
      decoration: TextDecoration.none);
  static final TextStyle subtitle5 = GoogleFonts.kanit(
      fontSize: 16,
      fontWeight: FontWeight.w600,
      color: red1,
      decoration: TextDecoration.none);
  static final TextStyle subtitle6 = GoogleFonts.kanit(
      fontSize: 16,
      fontWeight: FontWeight.w600,
      color: white3,
      decoration: TextDecoration.none);
  static final TextStyle subtitle7 = GoogleFonts.kanit(
      fontSize: 16,
      fontWeight: FontWeight.w600,
      color: black4,
      decoration: TextDecoration.none);
  static final TextStyle subtitle8 = GoogleFonts.kanit(
      fontSize: 16,
      fontWeight: FontWeight.w600,
      color: green1,
      decoration: TextDecoration.none);
  static final TextStyle subtitle9 = GoogleFonts.kanit(
      fontSize: 16,
      fontWeight: FontWeight.w500,
      color: black,
      decoration: TextDecoration.none);

//15
  static final TextStyle detail1 = GoogleFonts.kanit(
    fontSize: 15,
    fontWeight: FontWeight.w500,
    color: const Color.fromARGB(140, 112, 112, 112),
    decoration: TextDecoration.none,
  );
  static final TextStyle detail2 = GoogleFonts.kanit(
    fontSize: 15,
    fontWeight: FontWeight.w500,
    color: black4,
    decoration: TextDecoration.none,
  );

//14
  static final TextStyle text1 = GoogleFonts.kanit(
      fontSize: 14,
      fontWeight: FontWeight.w400,
      color: black6,
      decoration: TextDecoration.none);
  static final TextStyle text2 = GoogleFonts.kanit(
    color: black,
    fontSize: 14,
    fontWeight: FontWeight.w400,
    decoration: TextDecoration.none,
  );
  static final TextStyle text3 = GoogleFonts.kanit(
    color: black9,
    fontSize: 14,
    fontWeight: FontWeight.w400,
    decoration: TextDecoration.none,
  );
  static final TextStyle text4 = GoogleFonts.kanit(
      fontSize: 14,
      fontWeight: FontWeight.w400,
      color: black10,
      decoration: TextDecoration.none);
  static final TextStyle text5 = GoogleFonts.kanit(
    fontSize: 14,
    fontWeight: FontWeight.w500,
    decoration: TextDecoration.none,
    color: black1,
  );
  static final TextStyle text6 = GoogleFonts.kanit(
    fontSize: 14,
    fontWeight: FontWeight.w500,
    decoration: TextDecoration.none,
    color: black2,
  );
  static final TextStyle text7 = GoogleFonts.kanit(
    fontSize: 14,
    fontWeight: FontWeight.w500,
    decoration: TextDecoration.none,
    color: white3,
  );
  static final TextStyle text8 =
      GoogleFonts.kanit(fontSize: 14, fontWeight: FontWeight.w400, color: red1);
  static final TextStyle text9 = GoogleFonts.kanit(
      fontSize: 14,
      fontWeight: FontWeight.w400,
      color: black4,
      decoration: TextDecoration.none);
  static final TextStyle text10 = GoogleFonts.kanit(
      fontSize: 14,
      fontWeight: FontWeight.w500,
      color: black11,
      decoration: TextDecoration.none);
  static final TextStyle text11 = GoogleFonts.kanit(
      fontSize: 14,
      fontWeight: FontWeight.w400,
      color: black7,
      decoration: TextDecoration.none);
  static final TextStyle text12 = GoogleFonts.kanit(
    fontSize: 14,
    fontWeight: FontWeight.w500,
    color: const Color.fromARGB(124, 29, 27, 32),
    decoration: TextDecoration.none,
  );
  static final TextStyle text13 = GoogleFonts.kanit(
      fontSize: 14,
      fontWeight: FontWeight.w500,
      color: white3,
      decoration: TextDecoration.none);
  static final TextStyle text14 = GoogleFonts.kanit(
    fontSize: 14,
    fontWeight: FontWeight.w400,
    color: black12,
    decoration: TextDecoration.underline,
  );
  static final TextStyle text15 = GoogleFonts.kanit(
      fontSize: 14,
      fontWeight: FontWeight.w500,
      color: black4,
      decoration: TextDecoration.none);
  static final TextStyle text16 = GoogleFonts.kanit(
      fontSize: 14,
      fontWeight: FontWeight.w400,
      color: black13,
      decoration: TextDecoration.none);
  static final TextStyle text17 = GoogleFonts.kanit(
      fontSize: 14,
      fontWeight: FontWeight.w400,
      color: red1,
      decoration: TextDecoration.none);
  static final TextStyle text18 = GoogleFonts.kanit(
      fontSize: 14,
      fontWeight: FontWeight.w500,
      color: black4,
      decoration: TextDecoration.underline);
  static final TextStyle text19 = GoogleFonts.kanit(
      fontSize: 14,
      fontWeight: FontWeight.w500,
      color: red1,
      decoration: TextDecoration.none);
  static final TextStyle text20 = GoogleFonts.kanit(
    fontSize: 14,
    fontWeight: FontWeight.w300,
    color: black12,
    decoration: TextDecoration.underline,
  );

  //12
  static final TextStyle subtext1 = GoogleFonts.kanit(
      fontSize: 12, fontWeight: FontWeight.w400, color: black6);
  static final TextStyle subtext2 = GoogleFonts.kanit(
    color: blue1,
    fontSize: 12,
    fontWeight: FontWeight.w400,
    decoration: TextDecoration.underline,
    decorationColor: blue1,
  );
  static final TextStyle subtext3 = GoogleFonts.kanit(
      fontSize: 12, fontWeight: FontWeight.w400, color: black13);
  static final TextStyle subtext4 = GoogleFonts.kanit(
    fontSize: 12,
    fontWeight: FontWeight.w400,
    color: black7,
  );
  static final TextStyle subtext5 = GoogleFonts.kanit(
    color: black11,
    fontSize: 12,
    fontWeight: FontWeight.w400,
    decoration: TextDecoration.underline,
    decorationColor: black11,
  );
  static final TextStyle subtext6 = GoogleFonts.kanit(
    fontSize: 12,
    fontWeight: FontWeight.w400,
    color: red2,
  );
  static final TextStyle subtext7 = GoogleFonts.kanit(
      fontSize: 12, fontWeight: FontWeight.w500, color: black6);
  static final TextStyle subtext8 = GoogleFonts.kanit(
      fontSize: 12, fontWeight: FontWeight.w400, color: white3);
  static final TextStyle subtext9 = GoogleFonts.kanit(
      fontSize: 12,
      fontWeight: FontWeight.w400,
      color: red1,
      decoration: TextDecoration.underline,
      decorationColor: red1);
  static final TextStyle subtext10 = GoogleFonts.kanit(
    fontSize: 12,
    fontWeight: FontWeight.w400,
    color: red1,
  );

  //11
  static final TextStyle detailtext1 = GoogleFonts.kanit(
    fontSize: 11,
    fontWeight: FontWeight.w400,
    color: black6,
  );
  static final TextStyle detailtext2 = GoogleFonts.kanit(
    fontSize: 11,
    fontWeight: FontWeight.w400,
    color: white3,
  );
  static final TextStyle detailtext3 = GoogleFonts.kanit(
    fontSize: 11,
    fontWeight: FontWeight.w400,
    color: const Color.fromARGB(255, 81, 79, 79),
  );
  static final TextStyle detailtext4 =
      GoogleFonts.kanit(fontSize: 11, fontWeight: FontWeight.w400, color: red1);
  //10
  static final TextStyle subdetail1 = GoogleFonts.kanit(
    fontSize: 10,
    fontWeight: FontWeight.w400,
    color: black6,
  );
  static final TextStyle subdetail2 = GoogleFonts.kanit(
    fontSize: 10,
    fontWeight: FontWeight.w400,
    color: Colors.black,
  );
  static final TextStyle subdetail3 = GoogleFonts.kanit(
    fontSize: 10,
    fontWeight: FontWeight.w400,
    color: red1,
  );
}
